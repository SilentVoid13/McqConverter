    ...
    let matches = App::new("gift2amctxt")
        ...
        .arg(
            Arg::with_name("output")
                ...
                .help("Sets the output file name (default: [FILE].amctxt)")
        )
        .arg(
            Arg::with_name("comment")
                .short("c")
                .long("convert_comments")
                .help("Enables comments conversion")
        )
        .get_matches();
...
pub fn run(mcq_config: McqConfig, output: String) -> Result<(), Box<dyn Error>> {
    let mut converter = McqConverter::new(&mcq_config, Format::Gift)?;
    converter.convert_and_write_to(&output, Format::AmcTxt)?;
